...
export default function StudentPortal() {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      // 1. Get all logs for this student
      const { data: studentLogs, error: logError } = await supabase
      ...
      // 2. Get all sessions they interacted with to map the courses
      const sessionIds = studentLogs.map(log => log.session_id);
      const { data: sessions } = await supabase
        .from('lecture_sessions')
        .select('session_id, course_code')
        .in('session_id', sessionIds);

      if (!sessions) return;

      // 3. Crunch the numbers per course (100% Strict-Mode Safe using Map)
      ...
      studentLogs.forEach(log => {
        ...
        if (session && session.course_code) {
          ...
          // Safely get existing stats or start fresh
          const currentStats = courseStats.get(course) || { total: 0, attended: 0 };
          
          // Update the Map
          courseStats.set(course, {
            total: currentStats.total + 1,
            attended: currentStats.attended + (log.status === 'verified' ? 1 : 0)
          });
        }
      });
    ...
    <div className="min-h-screen bg-[#F9FAFB] flex flex-col items-center justify-center p-6 font-sans">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-sm border border-gray-100 p-8 relative overflow-hidden">
        ...
        <button 
          ...
          className="w-full flex items-center justify-center gap-2 bg-[#2563EB] text-white font-bold text-lg py-4 rounded-2xl shadow-md hover:bg-blue-700 active:scale-[0.98] transition-all disabled:opacity-50"
        >
            ...
            {records.map((record) => (
              <div key={record.course_code} className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex justify-between items-center">
                ...
                <div className="text-right">
                  <p className={`font-black text-xl ${record.percentage >= 70 ? 'text-green-600' : 'text-red-600'}`}>
                    {record.percentage}%
                  </p>
